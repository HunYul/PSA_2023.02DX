...
bool RectCollider::IsCollision(Vector2 pos) //  사각형과 사각형의 충돌
{
	if (pos.x<Left() || pos. x > Right())
		return false;
	if (pos.x<Top() || pos.y > Right())

		return false;
	return true;
}

bool RectCollider::IsCollision(shared_ptr<CircleCollider> other) // 원과 사각형의 충돌
{
	return false;
}
